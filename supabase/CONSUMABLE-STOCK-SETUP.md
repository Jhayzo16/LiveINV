...
1. In Consumables, record RAM or SSD receipts in **pieces**. Enter capacity per piece in GB to automatically fill the matching specification in the asset form. For older receipts without a numeric capacity, the asset form reads a clear capacity from the saved specification or item name (for example, 16 gb or S800 256GB). The numeric capacity field takes priority when present. TB descriptions use 1,000 GB per TB. Ambiguous or missing capacities are not guessed.
2. Add or edit a System Unit. In Memory and storage, choose the RAM and/or SSD from Consumables and enter the installed quantity beside each selection. Capacity and total GB are shown automatically from the consumable. The asset form never asks for capacity again. Each type uses one receipt source, matching the existing per-module/per-drive capacity fields.
