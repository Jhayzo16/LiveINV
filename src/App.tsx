...
export function App({ adminEmail, onSignOut }: { adminEmail?: string; onSignOut: () => Promise<void> }) {
  ...
  const [inventoryTool, setInventoryTool] = useState<'search' | 'alerts' | null>(null)
  const assetActionId = useRef(0)
  const [assetAction, setAssetAction] = useState<{ id: number; tag?: string; edit?: boolean } | null>(null)
  ...
  const openModule = (next: SystemModule | 'topology') => { setAssetAction(null); if (next !== 'assignments') setAssignmentTarget(null); setModule(next); resetToFloors(); window.scrollTo({ top: 0, behavior: 'auto' }) }
  const openAsset = (tag?: string, edit = false) => {
    setInventoryTool(null)
    openModule('assets')
    setAssetAction({ id: ++assetActionId.current, tag, edit })
  }
  ...
  return <div className={`app-shell module-${module} ${sidebarOpen ? '' : 'sidebar-collapsed'}`}>
    ...
    {inventoryTool && <InventoryTools mode={inventoryTool} assets={inventoryAssets} loading={inventoryLoading} error={Boolean(inventoryError)} onClose={() => setInventoryTool(null)} onSelect={openAsset} />}
    <Toaster />
  </div>
}
