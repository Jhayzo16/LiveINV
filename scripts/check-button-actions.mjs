...
try {
  ...
  await expect(page.locator('.topbar button').filter({ visible: true })).toHaveCount(0)
  ...
  console.log('PASS: quick registration, recent records, grid/list toggle, all report cards, filtered exports, sidebar-style hover, and responsive top-bar navigation.')
} catch (error) {
  mkdirSync('.tmp/button-actions', { recursive: true })
