...
try {
  ...
  await page.clock.setFixedTime(new Date('2026-09-16T00:05:00+08:00'))
  ...
  const dateTrigger = page.getByLabel('Maintenance date', { exact: true })
  await expect(dateTrigger).toContainText('September 16th, 2026')
  await dateTrigger.click()
  const calendar = page.getByRole('dialog', { name: 'Choose maintenance date' })
  ...
  await calendar.getByRole('button', { name: /Wednesday, August 12th, 2026/ }).click()
  await expect(calendar).not.toBeVisible()
  await expect(dateTrigger).toContainText('August 12th, 2026')
  mkdirSync('.tmp', { recursive: true })
  for (const width of [1440, 390, 320]) {
    await page.setViewportSize({ width, height: width === 1440 ? 1000 : 844 })
    await dateTrigger.click()
    const selectedDay = calendar.getByRole('button', { name: /Wednesday, August 12th, 2026, selected/ })
    await expect(selectedDay).toBeFocused()
    await selectedDay.press('ArrowRight')
    await expect(calendar.getByRole('button', { name: /Thursday, August 13th, 2026/ })).toBeFocused()
    const box = await calendar.boundingBox()
    expect(box.x).toBeGreaterThanOrEqual(0)
    expect(box.x + box.width).toBeLessThanOrEqual(width)
    expect(box.y).toBeGreaterThanOrEqual(0)
    expect(box.y + box.height).toBeLessThanOrEqual(width === 1440 ? 1000 : 844)
    await calendar.screenshot({ path: `.tmp/pms-calendar-${width}.png` })
    await page.keyboard.press('Escape')
    await expect(dateTrigger).toBeFocused()
    await expect(dateTrigger).toContainText('August 12th, 2026')
  }
  ...
  await expect(detail.locator('tbody tr')).toHaveCount(2)
  await expect(detail).toContainText('This session is complete')
  await expect(detail.getByRole('heading', { name: 'Session details' })).toBeVisible()
  await detail.getByRole('button', { name: 'View record for AP-OR-03' }).click()
  const savedRecord = detail.getByRole('region', { name: 'Maintenance record for AP-OR-03' })
  await expect(savedRecord).toContainText('IT Maintenance Team')
  await expect(savedRecord).toContainText('LIV-OR00003')
  await expect(savedRecord).toContainText('Aug 12, 2026')
  await page.screenshot({ path: '.tmp/pms-session-record-desktop.png', fullPage: true, animations: 'disabled' })
  await page.setViewportSize({ width: 390, height: 844 })
  await savedRecord.scrollIntoViewIfNeeded()
  await page.screenshot({ path: '.tmp/pms-session-record-mobile.png', animations: 'disabled' })
  expect(await page.locator('.pms-module').evaluate(element => element.scrollWidth <= element.clientWidth + 1)).toBe(true)
  await detail.getByRole('button', { name: 'Hide record for AP-OR-03' }).click()
  await page.setViewportSize({ width: 1440, height: 1000 })
